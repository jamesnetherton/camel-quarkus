...
You can control which routes get loaded at test time with configuration properties:
...
This can potentially have some unintended side effects for your tests. Specifically for 'application routes' (i.e. those not created by your test classes).
Modifications made to routes in one test will persist for following tests. For example when routes are modified with `AdviceWith` the advice will remain applied indefinitely.

Some ways to mitigating such issues are described below.

==== Forcing application restarts

You can force the `CamelContext` to restart between tests, by creating a custom https://quarkus.io/guides/getting-started-testing#testing_different_profiles[test profile].
...
TIP: In order to avoid needing to add an exponential number of `QuarkusTestProfile` classes, you can simply add `implements QuarkusTestProfile` to your test class.
...
Instead of forcing application restarts and to save time, there is an _experimental_ configuration option that can be added to `application.properties`.

[source]
----
quarkus.camel.test.reload-routes=true
----

This will cause all routes to be reset to their original state (i.e. as they were upon application startup) after each test is completed.
...
IMPORTANT: If you have routes configured that are NOT being advised, then you must invoke the `startRouteDefinitions` method after completing your `AdviceWith` setup.
