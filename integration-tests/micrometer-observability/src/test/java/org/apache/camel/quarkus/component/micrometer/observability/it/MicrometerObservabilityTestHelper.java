...
class MicrometerObservabilityTestHelper {
    private static final Logger LOG = Logger.getLogger(MicrometerObservabilityTestHelper.class);
    private static final boolean DEBUG_SPAN_HIERARCHY = ConfigProvider.getConfig().getValue("debug.span.hierarchy.enabled",
            boolean.class);

    private MicrometerObservabilityTestHelper() {
    }

    static List<Map<String, String>> getSpans() {
        List<Map<String, String>> spans = RestAssured.given()
                .get("/micrometer-observability/exporter/spans")
                .then()
                .statusCode(200)
                .extract()
                .body()
