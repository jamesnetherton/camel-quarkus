...
public class InfinispanDevModeZeroConfigTest {
    @RegisterExtension
    static final QuarkusDevModeTest TEST = new QuarkusDevModeTest()
            .setLogRecordPredicate(record -> record.getLevel().equals(Level.INFO))
            .setArchiveProducer(() -> ShrinkWrap.create(JavaArchive.class)
                    .addClass(CamelStartupObserver.class));

    @Test
    void defaultInfinispanClientProduced() {
        assertTrue(TEST.getLogRecords()
                .stream()
                .anyMatch(record -> record.getMessage().contains(LOG_MESSAGE + "true")));
    }
}
