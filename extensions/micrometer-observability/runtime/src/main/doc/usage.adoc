The extension automatically creates a Camel `MicrometerObservabilityTracer` and binds it to the Camel registry. It uses Micrometer's Observation API as an abstraction layer, with OpenTelemetry as the underlying tracing implementation via the `quarkus-micrometer-opentelemetry` extension.
...
Refer to the https://quarkus.io/guides/opentelemetry[Quarkus OpenTelemetry guide] and the https://quarkus.io/guides/telemetry-micrometer-to-opentelemetry[Quarkus Micrometer to OpenTelemetry guide] for a full list of configuration options.

Route endpoints can be excluded from tracing by configuring a property named `quarkus.camel.micrometer-observability.exclude-patterns` in `application.properties`. For example:

[source,properties]
----
# Exclude all direct & netty-http endpoints from tracing
quarkus.camel.micrometer-observability.exclude-patterns=direct:*,netty-http:*
----
